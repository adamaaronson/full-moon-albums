...
function getAddedDateString(album: Album) {
    return new Date(Date.parse(album.date_added))
...
export function AlbumCard({
    ...
    return (
        <div
            className={
                'album-card' +
                (isRecentlyAdded(album) ? ' album-card-recent' : '')
            }
        ...
        >
            {ranked ? (
                ...
                <div
                    className={
                        'album-date-label' +
                        (isRecentlyAdded(album) ? ' album-date-label-new' : '')
                    }
                >
                    <img className="album-date-label-moon" src="fullmoon.png" />
                    <div className="album-date-label-text">
                        {isRecentlyAdded(album) && !showingNewDate
                            ? 'NEW'
                            : getAddedDateString(album)}
                    </div>
                </div>
            )}
